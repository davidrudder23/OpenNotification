...
<%@ page import="net.reliableresponse.notification.aggregation.Squelcher" %>
...
<%
	...
	String numHoursString = (String) session.getAttribute("notification_squelched_hours");
	...
	String viewActiveString = request.getParameter("view_squelched_active");
	boolean viewActive = true;
	if (viewActiveString != null) viewActive = viewActiveString.toLowerCase().startsWith("t");

	String viewConfirmedString = request.getParameter("view_squelched_confirmed");
	boolean viewConfirmed = true;
	if (viewConfirmedString != null) viewConfirmed = viewConfirmedString.toLowerCase().startsWith("t"); 

	String viewExpiredString = request.getParameter("view_squelched_expired");
	boolean viewExpired = true;
	if (viewExpiredString != null) viewExpired = viewExpiredString.toLowerCase().startsWith("t"); 
	
	String viewOnholdString = request.getParameter("view_squelched_onhold");
	boolean viewOnhold = true;
	if (viewOnholdString != null) viewOnhold = viewOnholdString.toLowerCase().startsWith("t"); 
	%>
	<input type="hidden" name="view_squelched_active" value="<%= viewActive %>">
	<input type="hidden" name="view_squelched_confirmed" value="<%= viewConfirmed %>">
	<input type="hidden" name="view_squelched_expired" value="<%= viewExpired %>">
	<input type="hidden" name="view_squelched_onhold" value="<%= viewOnhold %>">
	<%
		
	List<Notification> recentNotifications = Squelcher.getSquelcherNotifications(user);
