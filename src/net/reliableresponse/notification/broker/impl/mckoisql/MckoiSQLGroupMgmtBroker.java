 ...
 */
public class MckoiSQLGroupMgmtBroker extends GenericSQLGroupMgmtBroker {
	...
	public int getGroups(int pageSize, int pageNum, Group[] groups) {
		...
		String sql = "SELECT m.uuid, g.membertype, m.firstname, m.description FROM member m, membergroup g WHERE m.deleted='N' AND m.uuid=g.uuid AND m.type='2' ORDER BY firstname";
	...
	String sql = "SELECT m.uuid FROM member m, membergroup g WHERE m.deleted='N' AND m.uuid=g.uuid AND m.type='2' AND lower(m.firstname) like ? ORDER BY firstname";
	...
	public int getGroupsUuidsLike(int pageSize, int pageNum, String substring,
		...
		String sql = "SELECT m.uuid FROM member m, membergroup g WHERE m.deleted='N' AND m.uuid=g.uuid AND m.type='2' AND lower(m.firstname) like ? ORDER BY m.firstname";
	...
	public int getGroupUuids(int pageSize, int pageNum, String[] uuids) {
		...
		String sql = "SELECT uuid FROM member WHERE deleted='N' AND type='2' ORDER BY firstname ";
