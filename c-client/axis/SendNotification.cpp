			...
			xsd__string uuid)
{
	...
	try
	{
		...
		m_pCall->addParameter((void*)&port, "port", XSD_INT);
		m_pCall->addParameter((void*)&uuid, "uuid", XSD_STRING);
		if (AXIS_SUCCESS == m_pCall->invoke())
		{
			if(AXIS_SUCCESS == m_pCall->checkMessage("sendNotificationResponse", url))
			{
...
int main(int argc, char** argv) {
	...
	SendNotification *sn;
	char *url, *recipient, *subject, *message;
	xsd__string result;

	if (argc < 4)
	{
		printf ("Usage: SendNotification url recipient subject message\n");
		return 0;
	}
	...
	result = sn->sendNotification (url, recipient, subject, message);
	printf ("%s", result);
